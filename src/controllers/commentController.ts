...
export const addComment = async (req: Request, res: Response) => {
    const { marker_id, comment } = req.body;
    ...
    try {
        ...
        const [existingComments] = await connection.query<RowDataPacket[]>(
            'SELECT id FROM MarkerComments WHERE marker_id = ? AND user_id = ?',
            [marker_id, userId]
        );
        ...
        await connection.query(
            'INSERT INTO MarkerComments (marker_id, user_id, comment) VALUES (?, ?, ?)',
            [marker_id, userId, comment]
        );
        connection.release();
...
export const updateComment = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await connection.query(
            'UPDATE MarkerComments SET comment = ? WHERE id = ?',
            [comment, comment_id]
        );
        connection.release();
